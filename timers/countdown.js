$(document).ready(function () {
    ...
    $("#startClockButton").on("click", startClock);
    // Assigns handler for the stop button
    $("#stopClockButton").on("click", stopClock);
});
...
/*
To cancel an interval, you need its ID (returned by the call to setInterval).
...
function updateClock() {
    ...
    let clockString, minutes, seconds;
...
function explode() {
    ...
    if (coinFlip < .5) {
        // ... we are not lucky
        $("#result").text("BOOM!").addClass("bang");
    }
    // ... and half the time
    else {
        /// ... we are
        $("#result").text("fzzzzz....").addClass("fizzle");
    }
}